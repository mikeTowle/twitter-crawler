 ...
 * @author mike
 */
public final class DepthFirstTweetTree implements TweetTree {

    static final Logger LOGGER = Logger.getLogger(DepthFirstTweetTree.class);
    private static DBClient db_instance;
    private final StatusRequest tweetRetriever;
    private TweetNode rootArticle;
    private final Stack<TweetNode> tweetStack;
     ...
     */
    public DepthFirstTweetTree(ArticleEntry ae, RepliesLinkedList ll, DBClient db) throws ClassNotFoundException, IOException {
        crawlStartTime = "";
        crawlEndTime = "";
        processed = 1;
        unProcessed = 0;
        ...
        tweetRetriever = new StatusRequest();
        tweetCount = 0;
    ...
    /**
     * initial call to recursive build() method
     *
    ...
    public void build() throws InterruptedException {
        ...
        addRepliesToStack(getRootArticle().getReplyList());
        LOGGER.info("Initiating Build");
        build(tweetStack.pop());
        tweetStack.clear();
        LOGGER.info("***************************Depth - First Construction Complete****************************");
    ...
    /**
     * recursive build method; drives construction
     *
     * @param x
     * @return
     * @throws InterruptedException
    ...
    private TweetNode build(TweetNode x) throws InterruptedException {
        populateReplyList(x);
        addRepliesToStack(x.getReplyList());
        int num_tweets = tweets.size();
        if (tweetStack.empty() || (num_tweets >= MAX_TWEET_COUNT)) { //timer
            Calendar cal = Calendar.getInstance();
            SimpleDateFormat sdf = new SimpleDateFormat("HH:mm:ss");
        ...
        return build(tweetStack.pop());
    }

    /**
     *
     * @param replyList added to global stack
     */
    private void addRepliesToStack(ArrayList<TweetNode> replyList) {
        tweetCount += replyList.size();
        unProcessed += replyList.size();
        for (int i = replyList.size() - 1; i >= 0; i--) {
            TweetNode temp = replyList.get(i);
            tweetStack.push(temp);
            getTweets().add(temp);
            tweets.add(temp);
        }
    }
    ...
    public void populateReplyList(TweetNode x) throws InterruptedException {
        ArrayList<Status> statusReplies = (ArrayList<Status>) tweetRetriever.queryTweet(x.getTweet());
        this.numQueries++;
        processed++; unProcessed--;
        queryAndCounts.put(numQueries, statusReplies.size());
    ...
    /**
     *
     * @param replies converted into a ArrayList
     * @return
     */
    @Override
    ...
    /**
     *
     * @param article to be root
     * @param ll article's initial query returned from Twitter
     */
    @Override
    public void buildFirstLevel(ArticleEntry article, RepliesLinkedList ll) {
        ...
        for (int i = 0; i < ll.getSize(); i++) {
            ...
            getDb_instance().insertTweet(temp.getTweet());
            tweets.add(temp);
        }
    }
